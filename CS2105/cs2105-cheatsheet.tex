...
\begin{itemize}
    ...
    \item If local DNS server does not have requested record in cache, it will start \textbf{recursive} DNS resolution process.
\end{itemize}
...
\subsection{Telnet}
\begin{itemize}
	\item Protocol to test open ports and enables terminal-to-terminal communication 
	\item Test connectivity, issue commands
\end{itemize}
...
\includegraphics[scale=0.20]{udp-socket}\\
\includegraphics[scale=0.20]{udp}
...
\includegraphics[scale=0.22]{tcp-socket}\\
\includegraphics[scale=0.3]{tcp}
...
\end{enumerate}
\begin{itemize}
    \item At receiver, segments are summed with checksum. All 1s if correct.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
        	\item Maximum send\slash receive window for N packets is 1\slash N
        \end{itemize}
...
\begin{itemize}
    ...
    \item Min window size = utilization * (2x+1)
    \begin{itemize}
    	\item x = prop delay
    \end{itemize}
\end{itemize}
...
\begin{description}	
	\item[Private IP addresses] {10.0.0.0\slash8, 172.16.0.0\slash12, 192,168.0.0\slash16}
	\begin{itemize}
		\item DHCP assigned IP address on LAN
	\end{itemize}
	\item[Loopback address] 127.0.0.0\slash8
	\item[Broadcast] 255.255.255.255\slash32 - local broadcast within subnet useful for discovering DHCP servers
	\item[Default network]0.0.0.0 - Any connection
	\item[Self-assigned]169.254.x.x - Device assigns itself if unable to receive IP from DHCP server
\end{description}
...
\begin{itemize}
	\item Runs over UDP, broadcasted in ethernet (FF:FF:.. dst mac address)
	\begin{itemize}
		\item Broadcast is used to ensure that all responding servers know that client has chosen a server
	\end{itemize}
...
\includegraphics[scale=0.26]{dhcp}
...
\begin{itemize}
	\item Fragmentation frequently occur in router but only reassembled at the destination
\end{itemize}
\includegraphics[scale=0.2]{ipv4-datagram-format}
\end{multicols*}
\begin{multicols*}{4}
\begin{itemize}
	\item Fragmentation does not replicate header of upper layer
	\item Checksum is the sum of all 16 bit segments in the header for verification of the header information
	\item Upper layer protocol eg. TCP, UDP, ICMP, IGMP
...
\begin{itemize}
	\item Stored as a table of $<$WAN IP, WAN port$>$ to $<$LAN IP, LAN port$>$ ~786,432 bytes
	\item Sending: Src ip $\rightarrow$ NAT ip of router, src port $\rightarrow$ random port (dst same)
	\item Return: Dst ip and port replaced with LAN ip and port saved
\end{itemize}
...
\begin{itemize}
	\item Routers maintain a routing table, with $<$subnet, next hop\slash interface$>$ pairings
	\item Routing tables determine the routes that packets go
	\item Routing algorithms populate routing tables
	\item Typically not load-balancing
	\begin{itemize}
		\item If cost is better, router will send all packets instead of splitting based on cost
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item Standard size MTU for ethernet is 1500 bytes (Excluding header of 18-20 bytes)
\end{itemize}
...
\begin{itemize}
	...
	\item Minimal time taken for the collision to be detected is RTT == $d_{transmission}$
\end{itemize}
...
\keyword{Hidden Node problem}{Interference between multiple transmitting terminals simultaneously transmitting data}
\begin{itemize}
	\item Receiving terminal unable to receive data from either terminals
\end{itemize}
\subsection{Ethernet}
\begin{itemize}
...
\section{11. Multimedia Networking}
\subsection{Types}
\subsubsection{Applications}
\begin{itemize}
	\item Streaming stored audio, video
	\begin{description}
		\item[Streaming]{Begin playout before downloading entire file}
		\item[Stored]{Can transmit faster than audio/video being rendered to be cached at user side}
	\end{description}
	\item Conversational ("2 way live") voice/video over IP
	\begin{itemize}
		\item Interactive nature with human-to-human conversation limit delay tolerance (sub 400ms)
	\end{itemize}
	\item Streaming ("one way live") audio/video
\end{itemize}
...
\begin{itemize}
	\item Compression to reduce data usage
	\item Redundancy within/btw images to reduce bits used
	\begin{itemize}
		\item Spatial coding (sending colour and number of pixels instead of duplicate pixels)
		\item Temporal coding (Sending difference betw frames instead of complete frames)
	\end{itemize}
\end{itemize}
\textbf{Bit rate}
\begin{description}
	\item[Constant bit rate]{Video encoding rate fixed}
	\begin{itemize}
		\item Not responsive to video complexity 
		\item Bitrate has to be set high by default to handle complex segments
		\item Consistent so well-suited for real-time encoding (streaming)
	\end{itemize}
	\item[Variable bit rate]{Video encoding rate changes as amount of spatial/temporal coding changes}
	\begin{itemize}
		\item Longer time to process data
		\item More suited for on-demand video 
	\end{itemize}
\end{description}

\subsubsection{Audio}
\begin{itemize}
	\item Analog audio signal sampled at constant rate
	\item Audio curve quantized to send through analog using converters
	\begin{itemize}
		\item \keyword{Quantized}{Rounding signal to be represented by x bits}
		\item sampling rate * x-bit representation $\rightarrow$ bit rate
	\end{itemize}
	\item \keyword{Channel}{Representation of sound coming from a single source (eg. 2 channel earphones for each ear)}
	\item \keyword{ADC/DAC}{Analog-to-Digital Converter (and vice versa)}
\end{itemize}

\subsection{Streaming}
\subsubsection{Objective}
\begin{description}
	\item[Continuous playout]{Playback must match original timing (even if network delays are variable)}
	\item[Client interactivity]{User may pause, fast-forward, rewind, jump through video etc}
	\item[Lost packets]{Video packets have to be retransmitted or just ignored}
\end{description}
\subsubsection{Mechanism}
\includegraphics[scale=0.2]{multimedia-streaming-buffering}
\begin{enumerate}
	\item Intial fill of buffer until playout begins
	\item Buffer fill level varies based on fill rate while keeping playout rate constant
\end{enumerate}
\begin{itemize}
	\item fill rate $>$ playout rate: buffer will not empty assuming delay is large enough to absorb variability in fill rate
	\begin{itemize}
		\item Initial playout delay tradeoff - buffer starvation less likely but user wait longer to start watching
	\end{itemize}
	\item fill rate $<$ playout rate: Buffer eventually empties (causing freezing of video playout until buffer fills)
\end{itemize}
\subsubsection{UDP}
\begin{itemize}
	\item Server sends at rate appropriate for client with short playout delay
	\item UDP is faster and no congestion control (transmission without rate control restrictions)
	\item Error is handled by application (decide what it wants to do)
	\item Video chunks encapsulated using RTP (Real Time Transport protocol)
	\begin{itemize}
		\item Maintains seq num, time stamps, video encoding
	\end{itemize}
	\item Control Connection maintained separately using RTSP (Real Time Streaming Protocol)
	\begin{itemize}
		\item Establish controlling media sessions btw endpoints eg. commands like play, record, pause
	\end{itemize}
\end{itemize}
\textbf{Drawbacks}
\begin{itemize}
	\item Need separate media control server like RTSP increasing cost and complexity
	\item UDP may not go through firewalls
\end{itemize}

\subsubsection{HTTP}
\begin{itemize}
	\item Multimedia file received via HTTP GET at maximum rate under TCP
\end{itemize}
\textbf{Advantages}
\begin{itemize}
	\item Passes more easily through firewalls
	\item Network infrastructure more tuned towards http/tcp connections
\end{itemize}
\textbf{Drawbacks}
\begin{itemize}
	\item Fill rate fluctuates due to TCP congestion control, retransmission (in order delivery)
	\item Larger playout delay 
\end{itemize}

\subsection{VoIP}
\begin{itemize}
	\item Voice over IP
	\item End-to-end delay requirement: Need to maintain conversational aspect 
	\begin{itemize}
		\item Higher delays $>$ 400ms noticeable, impairs interactivity
	\end{itemize}
	\item Minimum dataloss (conversation unintelligible)
\end{itemize}
\textbf{Challenge}
IP is a best-effort service that has no upperbound on delay and packet loss
\subsubsection{Characteristics}
\begin{itemize}
	\item Speaker only sends packets when user is speaking (in 20msec chunks)
	\item Application-layer header added to each chunk
	\item Chunk+header encapsulated under TCP/\textbf{UDP} segment
\end{itemize}
\subsubsection{Loss}
\begin{description}
	\item[Network loss]{Datagram lost due to network congestion (router buffer overflow or dropped or firewall)}
	\item[Delay loss]{Datagram arrives too late for playout at receiver}
	\item[Tolerance]{Depending on voice encoding/loss concealment, packet loss rates btw 1-10\% is tolerated}
\end{description}
\subsubsection{Fixed playout delay}
\includegraphics[scale=0.23]{multimedia-voip-fixed-delay}
\begin{itemize}
	\item Receiver attempts to playout each chunk at constant q msec after chunk generation
	\item Large q $\rightarrow$ less packet loss, small q $\rightarrow$ Better interactive experience
\end{itemize}
\subsubsection{Adaptive playout delay}
\includegraphics[scale=0.2]{adaptive-playout-delay}
\begin{itemize}
	\item Low playout delay, low late loss rate \textbf{during talking spurts}
	\item Estimate network delay and adjust playout delay at beginning of talk spurt
	\item Silent periods compressed and elongated to ensure chunks played out at constant rate during talking spurt
	\item Adaptively estimate packet delay 
	\begin{itemize}
		\item Exponential Weighted Moving Average calculated for every received packets
		\includegraphics[scale=0.24]{adaptive-playout-emwa-calc}
		\item $playout-time_i = t_i+d_i+4v_i$
...
\subsubsection{Motivation}
\begin{itemize}
	\item HTTP streaming is wasteful as needs large client buffer
	\item All client receives the same encoding of video despite variation in device/network bandwidth
\end{itemize}
\subsubsection{Mechanism}
\textbf{Server}
\begin{itemize}
	\item Divides video file into multiple chunks which is stored at encoded at different rates
	\item Manifest file provides URL for different encoding
\end{itemize}
\textbf{Client}
\begin{itemize}
	\item Periodically mesasures server-client bandwidth and consults manifest to request one chunk at a time
	\item Chooses maximum coding rate sustainable given current bandwidth (which will vary)
	\item Client determines when to request chunk, what encoding to request and the url server to request chunk from
\end{itemize}
\subsubsection{Implementation}
\begin{enumerate}
	\item Data is encoded into different qualities and cut into short segments
	\item Client downloads Manifest File which describes available videos and qualities
	\item Client/player executes adaptive bitrate algorithm (ABR) to determine segment to download next and plays it
\end{enumerate}
\subsubsection{Pros and cons}
\begin{itemize}
	\item Server is simple and has no firewall problems
	\item Standard web caching works
	\item \textbf{However} DASH based on media segment transmissions
	\item Only buffers a few segments at client side and does not provide low latency interactive 2way applications
\end{itemize}
...
\begin{itemize}
	...
	\item Deep into access networks at ISPs and close to users
	\item Service provider returns manifest which allows users to contact CDN for multimedia
\end{itemize}
\section{Summary}
\begin{center}
\begin{tabular}{|l|p{0.5in}|p{0.5in}|p{0.5in}|}
\hline
\textbf{Layer} & Transport & Network & Link\\
\hline
\textbf{Unit} & Packet & Fragment & Frame\\
\hline
\textbf{Service} & Process-to-Proc & End-to-End & Host-to-host\\
\hline
\textbf{Addressing} & Port Number & IP address & MAC address\\
\hline
\textbf{Devices} & End host & Routers & Switches\\
\hline
\textbf{Protocols} & TCP/UDP & IP, ICMP, DHCP & CSMA/CD, ARP\\
\hline
\end{tabular}
\end{center}
\end{multicols*}
