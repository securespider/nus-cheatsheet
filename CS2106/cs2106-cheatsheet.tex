...
\includegraphics[scale=0.2]{mem-vs-file}
...
\begin{itemize}
	...
	\item \keyword{Metadata}{Additional information associated with the file}
\end{itemize}
...
\end{description}
\includegraphics[scale=0.2]{ls-la}
...
\begin{description}
	...
	\end{itemize}
	\item[Direct access]{Same as random access but addressed to specific record}
\end{description}
...
\begin{description}
	...
	\item[Repositioning]{Seek (Move current position to new location)}
...
\end{itemize}
\includegraphics[scale=0.18]{c-file-operations}
...
\begin{description}
	...
	\item[Open count/reference count]{How many process this file opened}
\end{description}
\textbf{Criteria}
\begin{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
		\item Keep track of open files for process
		\item Points to system-wide open-file table entry
	\end{itemize}
	\item Sys-wide open-file table
	\begin{itemize}
		\item Keep track of open files in system 
		\item Each entry points to V-node entry
	\end{itemize}
	\item V-node table (virtual node)
	\begin{itemize}
		\item Link with file on physical drive
		\item Contains info about physical location of file on storage
	\end{itemize}
\end{enumerate}
\includegraphics[scale=0.17]{file-system-table-example}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Appears in multiple directory under same name referring to the same content using pointers
...
\begin{itemize}
	...
	\item Storage contains Master boot record and 1.. partitions
\end{itemize}
\subsubsection{MBR}
\begin{enumerate}
...
\begin{description}
	...
	\item[Partition details]{Metadata of partition/blocks free etc}
...
\begin{itemize}
	...
	\item External fragmentation because file size is fixed and specified in advance
\end{itemize}
\subsubsection{Linked list}
\begin{itemize}
	\item Linked list of disk blocks 
	\begin{itemize}
		\item Stores pointer to the next disk block number 
...
\begin{itemize}
	\item Use one block to save pointers of all the blocks needed in file (and index)
	\item Lower memory overhead (only index block of file needs to be in memory)
...
\begin{itemize}
	...
	\item \keyword{Bitmap}{Disk block represented by bits and 0/1 if free}
...
\begin{itemize}
	\item Each directory contains hash table indexed by filename (with chained collision resolution)
...
\begin{itemize}
	...
	\item Data block pointers in File Allocation Table with block info (free/next occupied block/damaged)
	\item OS cache table in Ram to facilitate linked list traversal
\end{itemize}
\subsubsection{Directory Structure}
\begin{itemize}
	\item Directory represented as a special file with linear list of directory entries
	\item \keyword{directory entry}{Fixed size,32 bytes containing name+extension, attributes (permissions, type), creation time, size+starting block}
	\begin{itemize}
		\item Name limited to 8+3 char for filename+extension
		\item Creation time limited to 1980-2107 within accuracy $\pm$2s (2+2 bytes, date+time)
		\item First block index using 12,16,32 for FAT12,16,32 respectively 
		\item Filesize in bytes (allocated 4 bytes)
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item File/directory described by Index Node (I-Node) containing file metadata and addresses
	\item \keyword{Superblock}{Describes whole FS including total num I-Node, I-Nodes per grp, total disk blocks + num in grps}
	\item \keyword{Group descriptors}{Describes block grp: num free blocks, num free I-Nodes. bitmap pointer}
	\begin{itemize}
		\item Superblock and descriptors duplicated in each block group for redundancy
	\end{itemize}
	\item \keyword{Bitmap}{Block/I-Node: keep track usage status free/occupied}
	\item \keyword{I-Node Table}{Array of I-nodes in particular block group accessible by unique index}
\end{itemize}
\subsubsection{I-Node}
\begin{itemize}
	\item Contains mode (2), owner info(4), file size(4/8), timestamps(3x4), data block pointers (15x4), reference count(2)
	\begin{description}
		\item[Direct Block]{1st 12 pointers to actual data}
		\item[Single Indirect block]{13th pointer to a disk block containing some direct pointers}
		\item[Double Indirect Block]{14th pointer to disk block with number of single indirect blocks}
		\item[Triple Indirect Block]{15th pointer to block containing double indirect blocks}
	\end{description}
	\item Fast access to small file and flexibility in handling big files
\end{itemize}
...
\begin{itemize}
	\item Hard to determine when to delete I-Node
	\begin{itemize}
		\item Can maintain I-Node reference count and decremented for every deletion
	\end{itemize}
\end{itemize}

\subsubsection{Sym Link}
\begin{itemize}
	\item Since only pathname is stored, link easily invalidated (during file name changes/deletion)
\end{itemize}
\end{multicols*}
\end{document}
