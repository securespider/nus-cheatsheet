...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Conduction of heat from fluid in tubes to storage cylinder
	\end{itemize}
	\item \keyword{Active} {Solar heated fluid is circulated by a fan or pump}
	\begin{itemize}
	 	\item Evacuated Tube Collector based heating system (active)
	\end{itemize} 
\end{itemize}
...
\begin{itemize}
	\item Voltage for max power increases
	\item Output current increase
	\item Open circuit voltage increases
\end{itemize}
\subsection{Silicon}
Types
\begin{itemize}
...
\begin{itemize}
	...
	\end{itemize}
	\item Pink arrow denotes conventional current (current flows from p- to n- side thr outer circuit when load is high)
\end{itemize}
...
\begin{itemize}
	...
	\item Along same latitude, there are 2 high tides and 2 low tides simultaneously
...
\begin{itemize}
	...
	\item Set up industries alongside OTEC power plant to increase efficiency
\end{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Produce Bioethanol from Starch (corn)
	\end{itemize}
\end{enumerate}
...
\begin{itemize}
	...
	\item Formation of the planet, \textbf{radioactive decay} of mineral, volcanic activity and from solar energy absorbed by surface
...
\begin{itemize}
	\item Electric Generation
	\item Geothermal Direct Space Heating
	\item Geothermal Direct Space Cooling
\end{itemize}
...
\section{09. Energy storage}
\subsection{Fuel Cells}
\begin{itemize}
	\item Electrochemical device that converts fuel into electric current
	\item Electricity generated from chemical reaction between fuel and oxidant
	\item Reactant flow into cell and reaction products flow out of the cell while electrolyte remains within it
	\item Consist of 3 parts: anode, electrolyte and cathode
	\begin{description}
		\item[Anode]{Contains catalyst made up of fine platinum powder that accelerates dissociation of fuels into electrons and ions}
		\item Fed on H2
		\item[Cathode]{Nickel catalyst that converts ions into waste chemicals like water or carbon dioxide}
		\item Fed on Oxygen/air
	\end{description}
\end{itemize}
\subsubsection{Polymer/Proton Electrolyte Membrane Fuel Cell}
\begin{itemize}
	\item Use solid polymer as electrolyte and porous carbon electrodes containing a platinum catalyst
	\item Operate at 80c with high-power density at low weight and volume
	\item Used for transportation applications due to fast startup time, low sensitivity to orientation and favourable power-to-weight ratio
	\item Difficulty in hydrogen storage 
	\begin{itemize}
		\item Hydrogen has low energy density so difficult to store enough hydrogen for long distance travel
	\end{itemize}
\end{itemize}
\includegraphics[scale=0.2]{pem-cell}
...
\subsubsection{From fossil fuels}
\begin{itemize}
	\item Steam Methane reforming
	\begin{itemize}
		\item Endothermic conversion of methane and water vapour into hydrogen and carbon dioxide
		\item $CH_4+H_2O+heat\rightarrow CO+3H_2$    (1)
		\item $CO+H_2O\rightarrow CO_2+H_2+heat$    (2)
	\end{itemize}
	\item Partial oxidation of natural gas
	\begin{itemize}
		\item Partial combustion of methane and oxygen gase to produce CO and H2
		\item Exothermic - no external heating required for more compact design of reactor
		\item $CH_4+\dfrac{1}{2}O_2\rightarrow CO + 2H_2 +heat$    (3)
	\end{itemize}
	\item Autothermal Reforming
	\begin{itemize}
		\item Combination of both reactions to maintain temperature of reactor
	\end{itemize}
	\item Gasification Process (Coal)
	\begin{itemize}
		\item Carbon converted to CO and H2
		\item Endothermic reaction that requires additional heat
		\item $C(s)+H_2O+heat\rightarrow CO + H_2$
	\end{itemize}
\end{itemize}

\subsubsection{Splitting water}
\begin{itemize}
	\item Water electrolysis
	\begin{itemize}
		\item $H_2O+electricity\rightarrow H_+\dfrac{1}{2}O_2$
	\end{itemize}
	\item Photo-electrolysis
	\item Photo-biological production
	\item High-temperature water decomposition
\end{itemize}

\subsection{Hydrogen Storage}
\subsubsection{Storage in Gaseous state}
\begin{itemize}
	\item Steel/low-weight composite tanks
	\begin{itemize}
		\item Require no internal heat exchange and commercially available
		\item Large physical volume required
		\item Rapid loss of H2 in accidents
	\end{itemize}
	\item \keyword{Cryogas}{Gaseous hydrogen cooled to near cryogenic temperature to increase volumetric energy density}
	\item Glass microspheres
	\begin{enumerate}
		\item Hollow glass spheres are filled with H2 at 350-700Bar and 300c by permeation in high-pressure vessel
		\item Microspheres cooled to room temperature and low-pressures
		\item Controlled release of H2 via gradual heating to 200-300c
	\end{enumerate}
	\begin{itemize}
		\item Low volumetric density and high pressure requirement for filling
		\item Slowly leak hydrogen at ambient temperatures
		\item High temperature requirement for H2 release
		\item Inherently safe because H2 is stored at relatively low pressures onboard
	\end{itemize}
\end{itemize}

\subsubsection{Storage in liquid state}
\begin{itemize}
	\item Cryogenic Hydrogen
	\begin{itemize}
		\item Higher gravimetric density and energy density at low pressures
		\item Boil-off loss during dormancy and requirement of super-insulated cryogenic containers for storage
	\end{itemize}
	\item Hydrogen as constituent in NaBH4 solutions
	\item Hydrogen as constituent in rechargeable organic liquids
\end{itemize}

\subsubsection{Storage in solid state}
\begin{itemize}
	\item Carbon and high surface materials
	\begin{itemize}
		\item H2 molecular physisorption/adsorption at cryogenic temperatures with extremely high surface area carbons
	\end{itemize}
	\item $H_2O$-reactive chemical hydrides
	\item Thermal chemical hydrides
	\item Rechargeable hydrides
\end{itemize}

\textbf{Advantages}
\begin{itemize}
	\item Higher purity H2 output
	\item Lower volume and pressure
	\item Not as commercially viable 
\end{itemize}

\subsection{Batteries}
\begin{itemize}
	\item Converts chemical energy to electrical energy via cathodes, anodes and electrodes
	\item Electrons spontaneously flow from negative to positive potential when electrodes are connected with external load
	\item Battery characteristics dependent on internal chemistry, drain and temperature
\end{itemize}
\subsubsection{Primary batteries}
\begin{itemize}
	\item Used once only as active material is consumed in single discharge via irreversible electrochemical reaction
	\item Low price, easy to carry due to low-weight and high energy density
	\item Zn-MnO2 and Li-MnO2
\end{itemize}
